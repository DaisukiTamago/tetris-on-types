...
type Multiply<A extends number, B extends number, Acc extends number = 0, Result extends unknown[] = []> =
    B extends Acc ?
        SizeOf<Result>
    : Multiply<A, B, Sum<Acc, 1>, [...Result, ...TupleOf<A>]>
...
type Div<A extends number, B extends number, Acc extends number = 0> =
    TupleOf<A> extends [...TupleOf<B>, ...infer Rest] ?
        Div<SizeOf<Rest>, B, Sum<Acc, 1>>
    : Acc

type Mod<A extends number, B extends number, Acc extends number = 0> =
    TupleOf<A> extends [...TupleOf<B>, ...infer Rest] ?
...
type isWithinInclusiveRange<Target extends number, Range extends { min: number, max: number }> =
    (IsGreaterThanOrEqual<Target, Range["min"]> & IsLessThanOrEqual<Target, Range["max"]>) extends never ?
        false
    : true
